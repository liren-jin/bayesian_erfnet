...
def precision_from_conf_matrices(
    ...
    if ignore_index is not None:
        per_class_precision = torch.cat(
            [
                per_class_precision[:ignore_index],
                per_class_precision[ignore_index + 1 :],
            ]
        )
...
def recall_from_conf_matrices(
    ...
    if ignore_index is not None:
        per_class_recall = torch.cat(
            [per_class_recall[:ignore_index], per_class_recall[ignore_index + 1 :]]
        )
...
def per_class_iou_from_conf_matrices(
    ...
    unions = [
        conf_mat.sum(0) + conf_mat.sum(1) - intersections[i]
...
def weighted_iou_from_conf_matrices(
    ...
    if ignore_index is not None:
        per_class_iou = torch.cat(
            [per_class_iou[:ignore_index], per_class_iou[ignore_index + 1 :]]
        )
...
def accuracy_from_conf_matrices(
    ...
    true_predictions = torch.stack(
        [torch.sum(torch.diag(conf_mat_masked(conf_mat))) for conf_mat in conf_matrices]
    )
    num_predictions = torch.stack(
        [torch.sum(conf_mat_masked(conf_mat)) for conf_mat in conf_matrices]
    )
...
def total_conf_matrix_from_conf_matrices(
    ...
    total_conf_matrix = total_conf_matrix / torch.sum(
        total_conf_matrix, dim=1, keepdim=True
    )
...
def compute_calibration_info(
    ...
    confidences, predictions = (
        torch.transpose(prediction_probs, 1, -1).flatten(0, -2).max(dim=1)
    )
...
def ece_from_calibration_info(calibration_info_list: List, num_bins: int = 20) -> float:
    ...
    return torch.sum(
        torch.abs(acc_bin - conf_bin) * prop_bin / torch.sum(prop_bin)
    ).item()
...
def compute_calibration_plots(outputs: List[Dict], num_bins: int = 20):
    ...
    ax = sns.lineplot(
        x=bin_boundaries.tolist(),
        y=bin_boundaries.tolist(),
        color="gray",
        linestyle="dashed",
        linewidth=3,
    )
    sns.lineplot(
        x=conf_bin.tolist(), y=acc_bin.tolist(), color="red", linewidth=3, ax=ax
    )
